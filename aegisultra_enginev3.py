...
def build_top_correct_scores(
    ...
    for period in ["FT", "HT"]:
        ...
        output[period] = scores

    return output


# ============================================================
# 15. Engine main pipeline
# ============================================================

def clean_public_candidate(
    candidate,
):
    output = {
        key: value
    ...
    output["scenarios"] = [
        public_scenario_record(item)
        for item in candidate["scenarios"]
    ]

    output["family_out_audit"] = dict(
        candidate["family_out_audit"]
    )

    output["family_out_audit"][
        "scenarios"
    ] = [
        public_scenario_record(item)
        for item
        in candidate["family_out_audit"][
            "scenarios"
        ]
    ]
...
def run_aegis_reconstruction(
    ...
    for book in validated["sharp_books"]:
        for period in ["FT", "HT"]:
    ...
    choose_recommendations(
        ...
        maximum_recommendations=settings[
            "max_recommendations"
        ],
        minimum_hit_probability=settings[
            "minimum_official_hit_probability"
        ],
    )

    official_recommendations = [
        clean_public_candidate(item)
        for item in evaluated_candidates
        if item["official"]
    ]

    all_candidates_public = [
        clean_public_candidate(item)
        for item in evaluated_candidates
    ]
    ...
    summary = {
        ...
        "official_recommendation_count": len(
            official_recommendations
        ),
    ...
    raw_output = {
        ...
        "coherence_audit": coherence_audit,
        "candidates": all_candidates_public,
        "reconstructions": {
            "FT": {
                "max_goals": period_results[
                    "FT"
                ]["max_goals"],
                "grid_complete": period_results[
                    "FT"
                ]["grid_complete"],
                "expansion_history": period_results[
                    "FT"
                ]["expansion_history"],
                "scenario_count": len(
                    period_results["FT"][
...
if __name__ == "__main__":
    ...
    output = run_aegis_reconstruction(
        sample_data
    )

    print(
        f"Engine: {output['engine']['name']} "
        f"v{output['engine']['version']}"
    )
    print(
        "Official Recommendations Count:",
        output["summary"][
            "official_recommendation_count"
        ],
    )
    for rec in output["official_recommendations"]:
        print(
            f" - Rank {rec['official_rank']}: "
            f"{rec['label']} @ {rec['hkjc_odds']} "
            f"(Min Hit: {rec['probability']['hit']['minimum']:.2%})"
        )
